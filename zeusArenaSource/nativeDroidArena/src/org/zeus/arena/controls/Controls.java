...
import org.zeus.arena.ControlLayout;
import org.zeus.arena.Persistence;
import org.zeus.arena.PlayerPreferences;
import org.zeus.arena.R;
import org.zeus.arena.R.id;
import org.zeus.arena.R.layout;

import org.zeus.arena.*;
...
public class Controls extends Activity{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final CheckBox turning = (CheckBox) findViewById(R.id.leftRight);
		turning.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
				if (turning.isChecked()){
					List<String> values = new Vector<String>();
					values.add("On");
					PlayerPreferences.getThePlayerPreferences().updatePreference("Turning", values);
				}
				else{
					List<String> values = new Vector<String>();
					values.add("Off");
	...
	private void checkPreferences(){
		...
		if(PlayerPreferences.getThePlayerPreferences().preferenceOn("Turning")){	//if turning is on
			CheckBox turning = (CheckBox) findViewById(R.id.leftRight);
			turning.setChecked(true);
		}
		else{
			CheckBox turning = (CheckBox) findViewById(R.id.leftRight);
			turning.setChecked(false);
		}
