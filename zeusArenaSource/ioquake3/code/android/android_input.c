...
static qboolean touch_event = qfalse;
static float touch_dx=0;
static float touch_dy=0;

static qboolean motion_event = qfalse;
static float motion_dx=0;
static float motion_dy=0;

static qboolean touchpad_event = qfalse;
static float touchpad_dx=0;
static float touchpad_dy=0;
...
void queueTouchEvent(int action, float x, float y, float pressure){
    //static float touch_prev_x=0, touch_prev_y=0;
    int t = Sys_Milliseconds();
    if (action == ACTION_DOWN){
        touch_event = qtrue;
	//float tempX = x - ((SCREEN_WIDTH-640)/2);	//107 
	float tempX = x;
	touch_dx = tempX;
        cursor_x = tempX;
	touch_dy = y;
...
void queueMotionEvent(int action, float x, float y, float pressure)
{
    ...
    {
        case ACTION_DOWN:
            /* When the positicase ACTION_DOWN:on change between ACTION_DOWN and ACTION_UP is small we 			generate a mouse click.
...
void queueTouchpadEvent(int action, float x, float y)
{
    ...
    switch(action)
    {
        ...
        case ACTION_MOVE:
            touchpad_event = qtrue;
            touchpad_dx += x;//-motion_initial_x;
            touchpad_dy += y;//-motion_initial_y;
            break;
    }
...
static void processMotionEvents()
{
	...
	int t = Sys_Milliseconds(); //what time should we use?
         ...
         * I have.
         */
	//Com_QueueEvent(t, SE_MOUSE, -854, -480, 0, NULL);
	Com_QueueEvent(t, SE_MOUSE, -glConfig.vidWidth, -glConfig.vidHeight, 0, NULL);
	//Com_QueueEvent(t, SE_MOUSE, scale_x_input(touch_dx), scale_y_input(touch_dy), 0, NULL);
	Com_QueueEvent(t, SE_MOUSE, (int)(touch_dx), (int)(touch_dy), 0, NULL);
	Com_QueueEvent(t, SE_KEY, K_MOUSE1, 1, 0, NULL);
	Com_QueueEvent(t, SE_KEY, K_MOUSE1, 0, 0, NULL);
    ...
    if(touchpad_event)
    {
        ...
        if(cls.state == CA_DISCONNECTED)
            ...
            Com_QueueEvent(t, SE_MOUSE, (int)(touchpad_dx), (int)(touchpad_dy), 0, NULL);
        else
            Com_QueueEvent(t, SE_MOUSE, (int)(touchpad_dx), (int)(touchpad_dy), 0, NULL);

        touchpad_event = qfalse;
        touchpad_dx = 0;
        touchpad_dy = 0;
    }
}
